...
@section('content')
    ...
    @php
        $cmsNew = $cms->where('id', 11)->first();
        $banner_image = $cmsNew ? $cmsNew->banner_image : null;
    @endphp

    <!-- Hero start -->
    <section class="hero search-hero" {{-- style="background-image: url('{{ asset('/frontend/assets/images/home-banner-bg.png') }}')"> --}}
                ...
                <form class="booking-form" action="{{ route('villa.search') }}" method="GET">
                    ...
                    <div class="form-element search-element">
                        ...
                        <button type="button" class="filter-icon">
                            <img src="{{ asset('/frontend/assets/images/Filters-icon.png') }}" alt="filter-icon"
                                width="30" height="30" />
                        </button>
                    </div>
    ...
    <main>
        ...
        <section class="section container villa-search-section listing-section">
            <div class="listing-header">
                <div class="listing-title-box">
                </div>
                <div class="listing-actions top-filter">
                    <select class="select2 villa-sort-option">
            ...
            <div class="listing-content">
                <div class="row g-md-3 g-xl-4">
                    <div class="col-lg-3 col-xl-4">
                        <div class="listing-filters responsive-filter">
                            ...
                            <div class="filter-range">
                                ...
                                <div class="range-wrapper">
                                    <input type="range" class="range-input min-range" min="100" max="100000"
                                        step="100" value="100" data-target="#minPrice" />
                                    <input type="range" class="range-input max-range" min="100" max="100000"
                                        step="100" value="100000" data-target="#maxPrice" />
                        ...
                        {{-- Rooms filter frontend start --}}
                            ...
                            <div class="filter-select">
                                <select class="select2 property-type-select">
                                    <option value="" selected disabled>{{ __('Select Property Type') }}</option>
                            ...
                            <div class="filter-selects">
                                ...
                                @endforeach
                                <button type="button" class="button button-pri filter-button mt-5"
                                    id="findVillasButton">
                    ...
                    <div class="col-lg-9 col-xl-8">
                                    ...
                                    <div class="villa-item">
                                        <div class="villa-item-top">
                                            ...
                                            <figure class="villa-item-img">
                                                <img src="{{ asset($villa->villa_images->first()->image ?? '/frontend/assets/images/exp-img-1.png') }}"
                                                    alt="{{ __($villa->title) ?? '' }}" />
                                            </figure>
                                        </div>
                                        <div class="villa-item-content">
                                            ...
                                            <div class="villa-item-bottom">
                                                <div class="price"><span>₪{{ $villa->price ?? '' }}</span> /
                                                    {{ __('nights') }}</div>
                                                <a class="button button-pri button-sm"
                                                    href="{{ route('villas.details', ['slug' => $villa->slug]) }}"><span>
                                                        {{ __('view details') }}
                                                    </span>
                                                    <svg class="icon" xmlns="http://www.w3.org/2000/svg" width="16"
                                                        height="16" viewBox="0 0 16 16" fill="none">
                                                        <path d="M9.61935 3.95331L13.666 7.99998L9.61935 12.0466"
                        ...
                        @else
                            <p class="text-center">{{ __('No Villa Found!') }}</p>
                        @endif
                        {{-- pagination start --}}
                        @if ($villas->count() > 0)
                            <div class="pagination-wrapper mt-4 mt-lg-5">
                                ...
                                @if ($end < $lastPage)
                                    @if ($end < $lastPage - 1)
                                        <span class="pagination-item pagination-more">...</span>
                                    @endif
                                    <a href="{{ $villas->url($lastPage) }}"
                                        class="pagination-item">{{ $lastPage }}</a>
                                @endif
                    ...
                    @foreach ($blogs->take(3) as $blog)
                        <div class="blog-item">
                            <figure class="blog-img">
                                <img src="{{ asset($blog->image ?? '/frontend/assets/images/blog-1.png') }}"
                                    alt="Blog image" />
                            </figure>
        ...
        function toggleBookmark(id, button) {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    showErrorToast('{{ __('An error occurred while processing your request.') }}');
                }
            });
        }
    </script>
            ...
            $('#findVillasButton').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        let villasList = $(".listing-results");
