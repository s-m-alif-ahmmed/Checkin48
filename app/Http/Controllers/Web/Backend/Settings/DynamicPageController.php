...
class DynamicPageController extends Controller {
    ...
    public function index(Request $request): View | JsonResponse {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->rawColumns(['page_title', 'status', 'action'])
                ->make();
        }
    ...
    public function store(Request $request): RedirectResponse {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'page_title.en'   => 'required|required_with:page_title.ar|string|max:100',
                'page_title.ar'   => 'required|required_with:page_title.en|string|max:100',

                'page_content' => 'required|array',
                'page_content.en' => 'required|required_with:page_content.ar|string',
                'page_content.ar' => 'required|required_with:page_content.en|string',
            ]);
            ...
            $data->page_slug    = Str::slug($request->page_title["en"]);
            $data->page_content = $request->page_content;
            $data->save();

            return redirect()->route('settings.dynamic_page.index')->with('t-success', 'Page create successfully');
        } catch (Exception) {
            return redirect()->route('settings.dynamic_page.index')->with('t-success', 'Page failed created.');
        }
    }
    ...
    public function update(Request $request, int $id): RedirectResponse {
        try {
            ...
            return redirect()->route('settings.dynamic_page.index')->with('t-success', 'Page Updated Successfully.');

        } catch (Exception) {
            return redirect()->route('settings.dynamic_page.index')->with('t-success', 'Page failed to update');
        }
    }
